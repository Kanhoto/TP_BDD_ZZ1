...
ERREUR a la ligne 1 :
ORA-00001: violation de contrainte unique (VICANHOTOD.PK_INSPECTEUR)

La clé primaire 2 est déjà utilisé
...
ERREUR a la ligne 1 :
ORA-02291: violation de contrainte d'integrite (VICANHOTOD.FKVNUM_TEST) - cle
parent introuvable'

la clé vnum 5 n'est pas présente dans la table VIN.'
...
ERREUR a la ligne 1 :
ORA-01400: impossible d'inserer NULL dans ("VICANHOTOD"."INSPECTEUR"."INOM")'

INOM de INSPECTEUR possède la contrainte NOTNULL

-- Q10-Supprimer le vin n°2. Que se passe-t-il ?  Pourquoi ?  

DELETE FROM VIN WHERE VNUM=2 ;
ERREUR a la ligne 1 :
ORA-02292: violation de contrainte (VICANHOTOD.FKVNUM_TEST) d'integrite -
enregistrement fils existant'

La suppression échoue, il faut d'abord supprimer les lignes dans test avec la clé fkvnum 2.'
...
ALTER TABLE TEST
ADD CONSTRAINT FKVNUM_TEST2 FOREIGN KEY(VNUM) REFERENCES VIN(VNUM) ON DELETE CASCADE;

ALTER TABLE TEST
DROP CONSTRAINT FKVNUM_TEST;
...
SELECT VIN.CEPAGE,VIN.VNUM,VIN.VNOM,TEST.NOTE,TEST.TDATE
FROM VIN,INSPECTEUR,TEST WHERE TO_CHAR(TEST.TDATE,'YYYY')='2009';

SELECT VNUM FROM VIN 
